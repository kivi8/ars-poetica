...
class UserManager extends Manager implements Nette\Security\IAuthenticator{
	 ...
	 */
        ...
        public function updatePassword($values, $userId){
            
            $oldPass = $this->userDat()->where('id', $userId)->fetch()->password;
            
            if(Passwords::verify($values->oldPassword, $oldPass)){
        ...
        public function updateUser($values, $user){
            ...
            if($values['photo']->isOK()){
                $photo = $this->genName($values['photo'], str_replace('@', '',str_replace('.', '',$user->mail)));
